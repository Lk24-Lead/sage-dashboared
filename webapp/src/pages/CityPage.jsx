...
// ── Filter bar ───────────────────────────────────────────────────
function FilterBar({ cities, selectedCities, onCities, hours, selectedHrs, onHrs, showHr, metricOptions, activeMetric, onMetric, lastFetch, loading, onRefresh }) {
  return (
...
function SummaryTable({ summary, cities, hours, hourly, metricOptions, activeMetric, onMetric, lastFetch, loading, onRefresh }) {
  ...
  let rows = summary
  if (selCities.length) rows = rows.filter(r => selCities.includes(r.city))

  if (selHrs.length) {
    rows = (selCities.length ? selCities : cities).map(city => {
      const cityHrs = (hourly[city] || []).filter(h => selHrs.includes(h.hr))
      if (!cityHrs.length) return null
      const sumKey = (getter) => cityHrs.reduce((a, h) => a + (getter(h) ?? 0), 0)
      ...
      const twCvr  = twSess ? cityHrs.reduce((a,h) => a + (h.tw.cvr ?? 0) * (h.tw.sessions ?? 0), 0) / twSess : null
      const lwCvr  = lwSess ? cityHrs.reduce((a,h) => a + (h.lw.cvr ?? 0) * (h.lw.sessions ?? 0), 0) / lwSess : null
  ...
  return (
    <>
      <FilterBar cities={cities} selectedCities={selCities} onCities={setSelCities}
      ...
      <div className="city-tbl-card">
        <div className="tbl-scroll">
          <table className="city-tbl">
            <thead>
              <tr className="thead-groups">
                <th className="th-city" rowSpan={2}>City</th>
                {METRICS.map(m => (
            ...
            <tbody>
              {sorted.map((r, i) => (
                <tr key={r.city || i}>
                  <td className="sticky-col td-city">{r.city || '—'}</td>
                  {METRICS.map(m => {
...
// ── Pivot table ──────────────────────────────────────────────────
function PivotTable({ metric, cities, hours, hourly, metricOptions, activeMetric, onMetric, lastFetch, loading, onRefresh }) {
  const m = METRICS.find(x => x.key === metric) || METRICS[0]
  const [selCities, setSelCities] = useState([])

  const visCities = selCities.length ? cities.filter(c => selCities.includes(c)) : cities

  function heatClass(val, cityRows) {
    if (val == null || !cityRows.length) return ''
    const vals = cityRows.map(h => h.tw[m.key]).filter(v => v != null && v > 0)
    ...
    <>
      ...
      <div className="city-tbl-card">
        <div className="pivot-header">
          <span className="pivot-title">{m.label} &mdash; City × Hour &nbsp;<span className="pivot-sub">TW · LW · Delta</span></span>
          <span className="pivot-hint">Shading = vs city's own hourly avg</span>
        </div>
        <div className="tbl-scroll">
          <table className="city-tbl pivot-tbl">
            <thead>
              <tr className="thead-groups">
                <th className="th-city" rowSpan={2}>City</th>
                {hours.map(hr => (
            ...
            <tbody>
              {visCities.map(city => {
                const cityRows = hourly[city] || []
                return (
                  <tr key={city}>
                    <td className="sticky-col td-city">{city}</td>
                    {hours.map(hr => {
                      const row = cityRows.find(h => h.hr === hr)
                      ...
                      const heat = heatClass(tw, cityRows)
                      return (
                        <Fragment key={hr}>
...
// ── Main ─────────────────────────────────────────────────────────
export default function CityPage({ cluster = 'india_next' }) {
  ...
  const fetchData = useCallback(async (cl) => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/city-data?cluster=${cl}`)
      if (res.status === 202) {
      ...
      <div className="city-content">
        ...
        {loading && !data && (
          <div className="loading-state">
            ...
            <p className="hint">City-level query takes ~45s on first load</p>
          </div>
        )}
        {loading && data && (
        ...
        {data && activeMetric === 'summary' && (
          <SummaryTable summary={data.summary} cities={data.cities} hours={data.hours} hourly={data.hourly}
        ...
        {data && activeMetric !== 'summary' && (
          <PivotTable metric={activeMetric} cities={data.cities} hours={data.hours} hourly={data.hourly}
            metricOptions={metricOptions} activeMetric={activeMetric} onMetric={setActiveMetric}
            lastFetch={lastFetch} loading={loading} onRefresh={() => fetchData(cluster)} />
        )}
        ...
        {data && (
          <div className="footer">
            <strong>Swiggy</strong> · SAGE OS · {CLUSTERS.find(c => c.key === cluster)?.label} City View · {data.generated_at}
          </div>
        )}
      </div>
    </div>
